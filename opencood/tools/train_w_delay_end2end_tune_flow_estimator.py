...
def main():
    ...
    if hypes['loda_pretrained_model']:
        # load pretrain_model
        main_model_dict = torch.load(hypes['pre_trained_path'])
        flow_pred_model_dict = torch.load(hypes['flow_pred']['model_file'])['model_state_dict']
        flow_model_dict_updated = OrderedDict()
        for key, value in flow_pred_model_dict.items():
            key_updated = 'backbone.defor_encoder.flow_pred.' + key
            flow_model_dict_updated[key_updated] = value
        main_model_dict.update(flow_model_dict_updated)
        model.load_state_dict(main_model_dict)

    # 载入预训练模型参数后...
    if hypes['finetune_flow_model']:
        for name, value in model.named_parameters():
            if name.startswith('backbone.defor_encoder.flow_pred.'):
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            if batch_data is None or batch_data['ego']['object_bbx_mask'].sum()==0:
                continue
            # when all rencord_len is 1, means only inf data; no calibrate; 
            if batch_data['ego']['record_len'].sum() == batch_data['ego']['record_len'].shape[0]:
                continue
