...
class CML(nn.Module):
    def __init__(self):
        ...
        self.conv3d_2 = Conv3d(64, 64, 3, s=(2, 1, 1), p=(1, 1, 1))
...
class RPN(nn.Module):
    def __init__(self, args):
        ...
        self.anchor_num = args['anchor_number']
        ...
        if 'dir_args' in args.keys():
            self.use_dir = True
            self.dir_head = nn.Conv2d(768, args['dir_args']['num_bins'] * args['anchor_number'],
    ...
    def forward(self, x):
        ...
        if self.use_dir:
            return self.score_head(x), self.reg_head(x), self.dir_head(x)
        else:
            return self.score_head(x), self.reg_head(x), None
...
class VoxelNet(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        psm, rm, dm = self.rpn(vwfs.view(self.N, -1, self.H, self.W))
