...
class AttResBEVBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        self.compress = False
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.num_levels):
            x_fuse = self.fuse_modules[i](x[i], record_len, pairwise_t_matrix)
            stride = int(spatial_features.shape[2] / x[i].shape[2])
            ret_dict['spatial_features_%dx' % stride] = x[i]
