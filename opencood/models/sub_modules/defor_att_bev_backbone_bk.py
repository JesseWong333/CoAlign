...
from opencood.models.fuse_modules.deformable_attn import DeforAttFusion
...
class DeforBEVBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        for idx in range(num_levels):
            ...
            fuse_network = DeforAttFusion(num_filters[idx])
        ...
        # decoder; added by Junjie Wang
        # 100, 252
        self.bev_h = 100 # magic number
        self.bev_w = 252
        self.embed_dims = 384  # 按照原来的大小设置
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(self.blocks)):
            ...
            if DEBUG:
                ...
                x_fuse = self.fuse_modules[i](x, record_len, pairwise_t_matrix)  # fuse的结果仅用于最后的预测

            stride = int(spatial_features.shape[2] / x.shape[2])
            ret_dict['spatial_features_%dx' % stride] = x

            if len(self.deblocks) > 0:
                ups.append(self.deblocks[i](x_fuse)) # 用了一层反卷积上采样，把通道变为一样； 就是fpn
            else:
                ups.append(x_fuse)
