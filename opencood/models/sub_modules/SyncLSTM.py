...
class SyncLSTM(nn.Module):
    def __init__(self, channel_size = 256, spatial_size = (32, 32), k = 3, TM_Flag = False, compressed_size = 64):
        super(SyncLSTM, self).__init__()
        self.k = k
        self.spatial_size = spatial_size
        self.channel_size = channel_size
        self.compressed_size = compressed_size
        self.lstmcell = MotionLSTM(spatial_size, self.compressed_size)
        self.init_c = nn.parameter.Parameter(torch.rand(self.compressed_size, spatial_size[0], spatial_size[1]))
...
class MotionLSTM(nn.Module):
    def __init__(self, spatial_size, input_channel_size, hidden_size = 0):
        ...
        self.b_i = nn.Parameter(torch.Tensor(1, self.input_channel_size, self.spatial_size[0], self.spatial_size[1]))
        ...
        self.b_f = nn.Parameter(torch.Tensor(1, self.input_channel_size, self.spatial_size[0], self.spatial_size[1]))
        ...
        self.b_c = nn.Parameter(torch.Tensor(1, self.input_channel_size, self.spatial_size[0], self.spatial_size[1]))
        ...
        self.b_o = nn.Parameter(torch.Tensor(1, self.input_channel_size, self.spatial_size[0], self.spatial_size[1]))
...
if __name__ == "__main__":
    dummy_input = torch.rand((1, 3, 128, 100, 252))
    delta_t = [2]
    syncnet = SyncLSTM(channel_size = 128, spatial_size = (100, 252), k = 3, TM_Flag = False, compressed_size = 128)
    xxx = syncnet(dummy_input, delta_t)
    pass
