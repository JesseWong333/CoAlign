...
def read_GT(inf_frame_id, pc_range=[-100.8, -40, -3.5, 100.8, 40, 1.5]):
...
def label_flow(data, bev_shape=[100, 252]):
    ...
    previous_infs = [data['previous_inf_'+str(i)] for i in range(1, track_n_frame + 1)]
    ...
    # update 11.2， Coalign的做法，是每个cav的检测范围是固定的，再去投影 （实际范围更小）；
    lidar_anchor, _ = pcd_utils.read_pcd(os.path.join(data_dir, anchor_inf))
    lidar_anchor = filter_point_cloud(lidar_anchor)
    lidar_l = [ filter_point_cloud(pcd_utils.read_pcd(os.path.join(data_dir, previous_inf[0]))[0]) for previous_inf in previous_infs if previous_inf is not None]
